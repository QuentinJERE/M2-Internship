...
class Traitement:
    def __init__(self,
        ...
        self.val_min_ppm_tmp=val_min_ppm_tmp
        self.val_max_ppm_tmp=val_max_ppm_tmp
        self.dic=None
        self.dic_save=None
        self.data=None
        self.pdic=None
    ...
    def complete(self):
        ...
        self.point_min=pd.DataFrame({self.column_1:np.linspace(self.val_min,self.val_min_ppm_tmp,self.truc_2),self.column_2:[self.p[0]+(self.p[1]*x_1[i])+(self.p[2]*x_1[i]**2)+complex(np.random.normal(self.m,self.e),np.random.normal(self.m_i,self.e_i)) for i in range(self.truc_2)]})
        x_2=np.linspace(10,self.val_max,self.truc)
        self.point_max=pd.DataFrame({self.column_1:np.linspace(self.val_max_ppm_tmp,self.val_max,self.truc),self.column_2:[self.p_2[0]+(self.p_2[1]*x_2[i])+(self.p_2[2]*x_2[i]**2)+complex(np.random.normal(self.m,self.e),np.random.normal(self.m_i,self.e_i)) for i in range(self.truc)]})
        ...
        self.point_min=pd.DataFrame({self.column_1:np.linspace(self.val_min,self.val_min_ppm_tmp,self.truc_2),self.column_2:[np.random.normal(self.m_proc,self.e_proc) for i in range(self.truc_2)]})
        self.point_max=pd.DataFrame({self.column_1:np.linspace(self.val_max_ppm_tmp,self.val_max,self.truc),self.column_2:[np.random.normal(self.m_proc,self.e_proc) for i in range(self.truc)]})
    ...
    def cut_2(self):
        '''Troncature de la plage de données'''
        self.cut_0=self.df3[self.df3[self.column_1]<self.val_min_ppm-0.1]
        self.cut_10=self.df3[self.df3[self.column_1]>self.val_max_ppm]
        self.df3=self.df3.drop(self.cut_0.index).drop(self.cut_10.index)
        self.cut_0=self.df3_proc[self.df3_proc[self.column_1]<self.val_min_ppm-0.1]
        self.cut_10=self.df3_proc[self.df3_proc[self.column_1]>self.val_max_ppm]
        self.df3_proc=self.df3_proc.drop(self.cut_0.index).drop(self.cut_10.index)
        if self.verbose == True:
            print(f"\033[38;2;255;0;255m\033[1m Découpe des spectres : \033[0m")
            print(f"\tOK\t\tOK")
        if self.disp_df == True:
...
class gen_mel:
    def __init__(self,
                 path="../spectres_csv/",
                 path_save="../spectres_csv/melange/",
                 column_1='x',
                 column_2='intensité',
                 nb_max=21800,
                 val_min_ppm=0.0,
        ...
        self.column_1=column_1
        self.column_2=column_2
        self.column_3="mélange"
        self.column_4="composes"
        self.nb_max=nb_max
        self.val_min_ppm=val_min_ppm
        self.val_max_ppm=val_max_ppm
        self.trace=trace
    ...
    def read_csv(self):
        self.df1=pd.read_csv(self.path_main+"spectres_ng/data_intensite.csv",sep=";",index_col=0)
        self.df1=self.df1.rename(columns={self.column_2:self.column_2+'_EtOH'})
        self.df1_proc=pd.read_csv(self.path_main+"spectres_proc/data_intensite.csv",sep=";",index_col=0)
        self.df1_proc=self.df1_proc.rename(columns={self.column_2:self.column_2+'_EtOH'})
        self.df2=pd.read_csv(self.path_main+"/data_proba.csv",sep=";",index_col=0)
        if self.verbose == True:
            print(f"\033[38;2;255;255;128m\033[1m Dossier charger : \033[0m")
            print(f"\t {self.path_main}")
            ...
            print(f"\tOK\t\tOK\tOK")
        if self.disp_df == True:
            display(self.df1,self.df1_proc,self.df2)
        
    def init_data_base(self):
        '''Création des tableaux de la base de données'''
        #nb_mel=self.df2.shape[0]**2+(self.df2.shape[0]/2)
        self.df4=pd.DataFrame(index=np.linspace(0,self.nb_max-1,self.nb_max).astype(int))
        self.df6=[]
        self.df4_proc=pd.DataFrame(index=np.linspace(0,self.nb_max-1,self.nb_max).astype(int))
    ...
    def create(self):
        self.melange=[]
        self.t=0
        for i in range(len(self.df2[self.column_4])):
            for j in range(i,len(self.df2[self.column_4])):
                self.melange.append(self.df2[self.column_4][i]+"_"+self.df2[self.column_4][j])
                path_save3=self.path_save+"spectres_ng/"+self.melange[self.t]+".csv"
                path_save4=self.path_save+"spectres_proc/"+self.melange[self.t]+".csv"
                path_save5=self.path_save+"spectres_ng/data_intensite.csv"
                path_save6=self.path_save+"spectres_proc/data_intensite.csv"
                rsuffix_int='_'+self.melange[self.t]
                self.df3=pd.DataFrame({self.column_1:np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.column_2:(self.mel[0]*self.df1[self.column_2+'_'+self.df2[self.column_4][i]]+self.mel[1]*self.df1[self.column_2+'_'+self.df2[self.column_4][j]])})
                self.df3_proc=pd.DataFrame({self.column_1:np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.column_2:(self.mel[0]*self.df1_proc[self.column_2+'_'+self.df2[self.column_4][i]]+self.mel[1]*self.df1_proc[self.column_2+'_'+self.df2[self.column_4][j]])})
                self.df6.append(list((np.array(self.df2[i:i+1])+np.array(self.df2[j:j+1]))[0][1:]))
                self.df4=self.df4.join(self.df3[self.column_2],rsuffix=rsuffix_int)
                self.df4_proc=self.df4_proc.join(self.df3_proc[self.column_2],rsuffix=rsuffix_int)
                self.df3.to_csv(path_save3,';')
                self.df3_proc.to_csv(path_save4,';')
                self.df4.to_csv(path_save5,';')
                self.df4_proc.to_csv(path_save6,';')
                if self.trace == True:
                    self.graph()
                if self.verbose == True:
                    print("==================================================================================================================================")
                    print(f"\t\t\t\t\t\t\033[1m---- {self.melange[self.t]} ----\033[0m")
                    print("==================================================================================================================================")
                    print(f"\033[38;2;0;255;255m\033[1m Sauvegarde des fichiers CSV : \033[0m")
                    print(f"\tOK\t\tOK\tOK")
                if self.disp_df == True:
                    display(self.df3,self.df3_proc)
                self.t+=1
        if self.disp_df == True:
            display(self.df4,self.df4_proc)

    def make_mel_proba(self):
        path_save7=self.path_save+"data_proba.csv"
        self.df7=pd.DataFrame({self.column_3:self.melange})
        self.df6=self.df7.join(pd.DataFrame(self.df6,columns=[self.df2[self.column_4]]).div(2))
        self.df6.to_csv(path_save7,';')
        if self.verbose == True:
            print(f"\033[38;2;128;0;255m\033[1m Sauvegarde du fichier CSV : \033[0m")
            print(f"\tOK\t\tOK\tOK")
        if self.disp_df == True:
            display(self.df6)
    ...
    def graph(self):
            '''Tracé de spectres à partir de fichier de données au format csv'''
            display(Markdown(f'<h4 style="background:black;color:cyan"><b> {self.melange[self.t]} </b></h4>'))
            fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
            graph.plot(self.df3[self.column_1],self.df3[self.column_2],lw=0.5,color='blue',zorder=1,alpha=1,label='Nmrglue spectrum')
            graph.plot(self.df3_proc[self.column_1],self.df3_proc[self.column_2],lw=0.5,color='lightgreen',zorder=2,alpha=1,label='TopSpin spectrum')
            graph.set_title(self.melange[self.t]+" NMR spectra")
            graph.set_xlabel("Chemical Shift (ppm)")
            graph.set_ylabel("Signal normalized")
            graph.set_ylim(-0.05,1e0)
            graph.axhline(0,-1e4,1e8,linestyle=':',color='black',alpha=1,zorder=-10)
...
class reload_mel_db:
    ...
    def read_csv(self):
        if self.path_one!=None:
            self.df1=pd.read_csv(self.path_main+'spectres_proc/'+self.path_one+'.csv', sep=";",index_col=0)
            self.df2=pd.read_csv(self.path_main+'spectres_ng/'+self.path_one+'.csv', sep=";",index_col=0)
        else:
            self.df1=pd.read_csv(self.path_main+'spectres_proc/data_intensite.csv', sep=";",index_col=0)
            self.df1=self.df1.rename(columns={self.column_2:self.column_2+'_EtOH_EtOH'})
            self.df2=pd.read_csv(self.path_main+'spectres_ng/data_intensite.csv', sep=";",index_col=0)
            self.df2=self.df2.rename(columns={self.column_2:self.column_2+'_EtOH_EtOH'})
        if self.verbose == True:
            print(f"\033[38;2;255;255;128m\033[1m Dossier charger : \033[0m")
            print(f"\t {self.path_main}")
            print(f"\033[38;2;0;255;0m\033[1m Chargement des fichiers CSV : \033[0m")
            print(f"\tOK\t\tOK\tOK")
        if self.disp_df == True:
            display(self.df1,self.df2)

    def graph(self):
        if self.path_one!=None:
            display(Markdown(f'<h4 style="background:black;color:cyan"><b> Mélange {self.path_one} </b></h4>'))
            fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1,lw=0.75,color='green',zorder=10,alpha=1,label='spectre melange theorique TopSpin')#
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2,lw=0.5,color='blue',zorder=9,alpha=1,label='spectre melange theorique nmrglue')#
            graph.set_xlabel("Chemical Shift (ppm)")
            graph.set_ylabel("Signal normalized")
            ...
            for i in self.df1.columns:
                display(Markdown(f'<h4 style="background:black;color:cyan"><b> Mélange {i[10:]} </b></h4>'))
                fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
                graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1[i],lw=0.75,color='green',zorder=10,alpha=1,label='spectre melange theorique TopSpin')#
                graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2[i],lw=0.5,color='blue',zorder=9,alpha=1,label='spectre melange theorique nmrglue')#
                graph.set_xlabel("Chemical Shift (ppm)")
                graph.set_ylabel("Signal normalized")
                graph.set_ylim(0,1e0)
                graph.axhline(0,-1e4,1e8,linestyle=':',color='black',zorder=-10)
...
class reload_db:
    ...
    def read_csv(self):
        if self.path_one!=None:
            ...
            self.df1=self.df1.rename(columns={self.column_2:self.column_2+'_EtOH'})
            self.df2=pd.read_csv(self.path_main+'spectres_ng/data_intensite.csv', sep=";",index_col=0)
            self.df2=self.df2.rename(columns={self.column_2:self.column_2+'_EtOH'})
        if self.verbose == True:
            print(f"\033[38;2;255;255;128m\033[1m Dossier charger : \033[0m")
            print(f"\t {self.path_main}")
            print(f"\033[38;2;0;255;0m\033[1m Chargement des fichiers CSV : \033[0m")
    ...
    def graph(self):
        if self.path_one!=None:
            display(Markdown(f'<h4 style="background:black;color:cyan"><b> Spectres {self.path_one} </b></h4>'))
            fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1,lw=0.75,color='green',zorder=10,alpha=1,label='spectre TopSpin')#
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2,lw=0.5,color='blue',zorder=9,alpha=1,label='spectre nmrglue')#
            graph.set_xlabel("Chemical Shift (ppm)")
            graph.set_ylabel("Signal normalized")
            graph.set_ylim(0,1e0)
            graph.axhline(0,-1e4,1e8,linestyle=':',color='black',zorder=-10)
            ...
            for i in self.df1.columns:
                display(Markdown(f'<h4 style="background:black;color:cyan"><b> Spectres {i[10:]} </b></h4>'))
                fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
                graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1[i],lw=0.75,color='green',zorder=10,alpha=1,label='spectre TopSpin')#
                graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2[i],lw=0.5,color='blue',zorder=9,alpha=1,label='spectre nmrglue')#
                graph.set_xlabel("Chemical Shift (ppm)")
                graph.set_ylabel("Signal normalized")
                graph.set_ylim(0,1e0)
                graph.axhline(0,-1e4,1e8,linestyle=':',color='black',zorder=-10)
...
class gen_decal:
    def __init__(self,
                 path="../spectres_csv/",
                 path_save="../spectres_csv/decalage/",
                 column_1='x',
                 column_2='intensité',
                 nb_max=21800,
                 val_min_ppm=0.0,
        ...
        self.column_1=column_1
        self.column_2=column_2
        self.ppm_shift=ppm_shift
        self.random_ppm_shift=random_ppm_shift
        self.nb_spectres=nb_spectres
        self.nb_max=nb_max
        self.val_min_ppm=val_min_ppm
        self.val_max_ppm=val_max_ppm
        self.trace=trace
    ...
    def init_data_base(self):
        ...
        self.df3=self.df2
        self.df6=self.df1
        self.df6_proc=self.df1_proc
        self.df4_proc=pd.DataFrame(index=np.linspace(0,self.nb_max-1,self.nb_max).astype(int))
        self.df5_proc=pd.DataFrame(index=np.linspace(0,self.nb_max-1,self.nb_max).astype(int))

    def creat_decal(self):
        path_save1=self.path_save+'spectres_ng/'+str(self.ppm_shift)+'_'+str("%0.3f" % self.random_ppm_shift)+'_'+str(self.nb_spectres)+'.csv'
        path_save2=self.path_save+'spectres_proc/'+str(self.ppm_shift)+'_'+str("%0.3f" % self.random_ppm_shift)+'_'+str(self.nb_spectres)+'.csv'
        for i in range(self.nb_spectres):
            self.random_ppm_shift_pos=random.uniform(-1,1)*self.random_ppm_shift
            self.random_ppm_shift_neg=random.uniform(-1,1)*self.random_ppm_shift

            nb_pos=int((((self.ppm_shift+self.random_ppm_shift_pos)*(i+1))*self.nb_max)//(self.val_max_ppm*self.nb_spectres))
            tmp_pos=self.df1.iloc[self.nb_max-nb_pos:]
            self.df4=self.df1.shift(periods=nb_pos,axis=0)
            self.df4.iloc[:nb_pos]=tmp_pos
            tmp_pos_proc=self.df1_proc.iloc[self.nb_max-nb_pos:]
            self.df4_proc=self.df1_proc.shift(periods=nb_pos,axis=0)
            self.df4_proc.iloc[:nb_pos]=tmp_pos_proc

            nb_neg=int(self.nb_max-((((self.ppm_shift+self.random_ppm_shift_neg)*(i+1))*self.nb_max)//(self.val_max_ppm*self.nb_spectres)))
            nb_pos_neg=self.nb_max-nb_neg
            tmp_neg=self.df1.iloc[:nb_pos_neg]
            self.df5=self.df1.shift(periods=-nb_pos_neg,axis=0)
            self.df5.iloc[nb_neg:]=tmp_neg
            tmp_neg_proc=self.df1_proc.iloc[:nb_pos_neg]
            self.df5_proc=self.df1_proc.shift(periods=-nb_pos_neg,axis=0)
            self.df5_proc.iloc[nb_neg:]=tmp_neg_proc

            self.df3=self.df3.join(self.df2.drop(['composes'],axis=1),rsuffix=("_-"+str(((self.ppm_shift+self.random_ppm_shift_neg)*(i+1))/self.nb_spectres)))
            self.df3=self.df3.join(self.df2.drop(['composes'],axis=1),rsuffix=("_+"+str(((self.ppm_shift+self.random_ppm_shift_pos)*(i+1))/self.nb_spectres)))
            self.df6=self.df6.join(self.df4,rsuffix=("_-"+str(((self.ppm_shift+self.random_ppm_shift_neg)*(i+1))/self.nb_spectres)))
            self.df6=self.df6.join(self.df5,rsuffix=("_+"+str(((self.ppm_shift+self.random_ppm_shift_pos)*(i+1))/self.nb_spectres)))
            self.df6_proc=self.df6_proc.join(self.df4_proc,rsuffix=("_-"+str(((self.ppm_shift+self.random_ppm_shift_neg)*(i+1))/self.nb_spectres)))
            self.df6_proc=self.df6_proc.join(self.df5_proc,rsuffix=("_+"+str(((self.ppm_shift+self.random_ppm_shift_pos)*(i+1))/self.nb_spectres)))
        self.df3.to_csv(self.path_save+'data_proba_'+str(self.ppm_shift)+'_'+str("%0.3f" % self.random_ppm_shift)+'_'+str(self.nb_spectres)+'.csv',';')
        self.df6.to_csv(path_save1,';')
        self.df6_proc.to_csv(path_save2,';')
        if self.verbose == True:
            print(f"\033[38;2;0;255;255m\033[1m Sauvegarde des fichiers CSV : \033[0m")
            print(f"\tOK\t\tOK\tOK")
        if self.disp_df == True:
            display(self.df6,self.df6_proc,self.df3)

    def graph(self):
            '''Tracé de spectres à partir de fichier de données au format csv'''
            display(Markdown(f'<h4 style="background:black;color:cyan"><b> Décalage de {self.ppm_shift} en {self.nb_spectres} spectres </b></h4>'))
            fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df6.filter(regex=self.column_2+'_+',axis=1),lw=0.5,zorder=3,color='blue',alpha=1,label='Spectres nmrglue décalés positivement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df6.filter(regex=self.column_2+'_-',axis=1),lw=0.5,zorder=3,color="red",alpha=1,label='Spectres nmrglue décalés négativement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df6.filter(regex=self.column_2,axis=1),lw=0.5,zorder=4,color='lawngreen',alpha=1,label='Spectre nmrglue original')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df6_proc.filter(regex=self.column_2+'_+',axis=1),lw=0.5,zorder=-3,color='cyan',alpha=1,label='Spectres TopSpin décalés positivement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df6_proc.filter(regex=self.column_2+'_-',axis=1),lw=0.5,zorder=-3,color="magenta",alpha=1,label='Spectres TopSpin décalés négativement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df6_proc.filter(regex=self.column_2,axis=1),lw=0.5,zorder=-3,color='yellow',alpha=1,label='Spectre TopSpin original')
            graph.set_title(" NMR spectra shifted to"+str(self.ppm_shift))
            graph.set_xlabel("Chemical Shift (ppm)")
            graph.set_ylabel("Signal normalized")
            graph.set_ylim(-0.05,1e0)
            graph.axhline(0,-1e4,1e8,linestyle=':',color='black',alpha=1,zorder=-10)
...
class reload_decal_db:
    def __init__(self,
                 path="../spectres_csv/decalage/",
                 path_one=None,
                 column_1='x',
                 column_2='intensité',
                 nb_max=21800,
    ...
    def read_csv(self):
        self.df1=pd.read_csv(self.path_main+'spectres_proc/'+self.file_path, sep=";",index_col=0)
        self.df2=pd.read_csv(self.path_main+'spectres_ng/'+self.file_path, sep=";",index_col=0)
        if self.verbose == True:
            print(f"\033[38;2;255;255;128m\033[1m Dossier charger : \033[0m")
            print(f"\t {self.path_main}")
            print(f"\033[38;2;0;255;0m\033[1m Chargement des fichiers CSV : \033[0m")
    ...
    def graph(self):
        for i in self.df1.columns[:29]:
            display(Markdown(f'<h4 style="background:black;color:cyan"><b> Spectres {i[10:]} décalé de {self.file_path[:-3]} </b></h4>'))
            fig, graph = plt.subplots(figsize=(1500/72,8),layout="constrained")
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1.filter(regex=i+'_+',axis=1),lw=0.5,zorder=1,color='blue',alpha=1,label='Spectres nmrglue décalés positivement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1.filter(regex=i+'_-',axis=1),lw=0.5,zorder=1,color="red",alpha=1,label='Spectres nmrglue décalés négativement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df1[i],lw=0.5,zorder=-3,color='lawngreen',alpha=1,label='Spectre nmrglue original')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2.filter(regex=i+'_+',axis=1),lw=0.5,zorder=1,color='cyan',alpha=1,label='Spectres TopSpin décalés positivement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2.filter(regex=i+'_-',axis=1),lw=0.5,zorder=1,color="magenta",alpha=1,label='Spectres TopSpin décalés négativement')
            graph.plot(np.linspace(self.val_max_ppm,self.val_min_ppm,self.nb_max),self.df2[i],lw=0.5,zorder=-3,color='yellow',alpha=1,label='Spectre TopSpin original')
            graph.set_xlabel("Chemical Shift (ppm)")
            graph.set_ylabel("Signal normalized")
            graph.set_ylim(0,1e0)
            graph.axhline(0,-1e4,1e8,linestyle=':',color='black',zorder=-10)
    ...
    def reload(self):
        if self.path_one!=None:
            self.file_path=self.path_one
            self.read_csv()
            if self.trace == True:
                self.graph()
        else:
            self.path=self.path_main+'spectres_ng/'
            for self.file_path in os.listdir(self.path):
                print("==================================================================================================================================")
                print(f"\t\t\t\t\t\t\033[1m---- {self.file_path[:-3]} ----\033[0m")
                print("==================================================================================================================================")
                self.read_csv()
                if self.trace == True:
                    self.graph()
